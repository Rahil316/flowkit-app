...
| 32  | FlowStory CLI (`flowStory:ls`, `check:flowStories`)                           | Workspace-Setup | 1     | done    | `scripts/platform/flowStoryDiscovery.js` (discovery) + `scripts/checks/flowStories.js` (validation); format-aware (flat + old nested); `check:flowStories` wired into `prebuild`. Row otherwise stale (references defunct `scripts/lib/` paths elsewhere in this table) — needs a full pass, out of scope here                                                                                                                                                                                                                                             |
...
| 39  | CLI contract smoke tests (flowStory:ls/check:flowStories/status/lens/migrate) | Workspace-Setup | 1     | done    | Row stale — referenced `scripts/tests/plan-contract-cli.test.js` no longer exists; needs a full pass, out of scope here                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| 43  | Vite plugin (`flowkit/vite`) — virtual modules, HMR                           | Package-Publish | 1     | done    | `scripts/vite-plugin.js`; `virtual:flowkit/pages\|config\|workspace\|flowStories`; this repo's own `vite.config.ts` consumes it                                                                                                                                                                                                                                                                                                                                                                                                                            |
