...
- `npm run build` — tsc + vite build (runs `flowkit check:flowStories` prebuild gate automatically)
...
- **`prebuild` gate** — `npm run build` always runs `flowkit check:flowStories`; exits non-zero on blocking plan issues
